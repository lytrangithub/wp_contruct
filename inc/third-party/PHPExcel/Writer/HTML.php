...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	public function generateNavigation()
	{
		...
		if (count($sheets) > 1) {
			...
			foreach ($sheets as $sheet) {
				$html .= '  <li><a>' . $sheet->getTitle() . '</a></li>' . PHP_EOL;
				++$sheetId;
			}
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($this->_phpExcel->getCellXfCollection() as $index => $style) {
			$css['td' . $index] = $this->_createCSSStyle( $style );
			$css['th' . $index] = $this->_createCSSStyle( $style );
		}
			...
			foreach ($pValues as $cellAddress) {
				...
				if ($cell instanceof PHPExcel_Cell) {
					...
					if (!$this->_useInlineCss) {
                        ...
                        // if ($cellType == 'th') {
                        //     if (isset($this->_cssStyles['th' . $cell->getXfIndex()])) {
                        //         $cssClass = array_merge($cssClass, $this->_cssStyles['th' . $cell->getXfIndex()]);
                        //     }
                        // } else {
                        //     if (isset($this->_cssStyles['td' . $cell->getXfIndex()])) {
                        //         $cssClass = array_merge($cssClass, $this->_cssStyles['td' . $cell->getXfIndex()]);
                        //     }
                        // }
				...
				// Write
				if ($writeCell) {
					$cellData = (!empty($cellData)) ? trim($cellData) : '';
					if ($cellData) {
						$html .= '<p>';
						$html .= $cellData;
						// Column end
